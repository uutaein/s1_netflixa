...
export default {
  ...
  methods: {
    getComments () {
      ...
      const apiUrl = baseUrl + '/reviews/' + this.id + '/comments'
    ...
    createComment () {
      ...
      form.append('content', this.word)
      const apiUrl = baseUrl + '/reviews/' + this.id + '/comments/' + 'create/'
      this.$http.post(apiUrl, form, config)
        .then(res => {
          console.log(res)
          res.data.created_at = (
